...
void findAntinode(std::vector<std::vector<char>>& map, char character);
...
void findAntinode(std::vector<std::vector<char>>& arr, char character) {
    ...
    for (int i = 0; i < position.size()-1; i++) {
        for (int j = i+1; j < position.size(); j++) {
            ...
            while (posY1 >= 0 && posX1 >= 0 && posX1 < arr[0].size()) {
                if (arr[posY1][posX1] == '.') {
                    arr[posY1][posX1] = '#';
                }
            
                posY1 -= std::abs(position[i][0] - position[j][0]);

                if (position[i][1] > position[j][1]) {
                    posX1 += position[i][1] - position[j][1];
                } else {
                    posX1 -= std::abs(position[i][1] - position[j][1]);
                }
            }
            while (posY2 < arr.size() && posX2 >= 0 && posX2 < arr[0].size()) {
                if (arr[posY2][posX2] == '.') {
                    arr[posY2][posX2] = '#';
                }

                posY2 += std::abs(position[i][0] - position[j][0]);

                 if (position[i][1] > position[j][1]) {
                    posX2 -= std::abs(position[i][1] - position[j][1]);
                } else {
                    posX2 += std::abs(position[i][1] - position[j][1]);
                }
            }
