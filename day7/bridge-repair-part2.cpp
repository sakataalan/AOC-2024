...
void combinationsGenerator(int size, std::vector<std::string> current, std::vector<std::vector<std::string>>& result);
std::uint64_t tryToFind(std::vector<int> arr, std::vector<std::vector<std::string>>& combinations, std::uint64_t target);
int main() {
    ...
    for (int i = 0; i < combinations.size(); i++) {
        ...
        for (int j = 0; j < combinations[i].size(); j++) {
            if (combinations[i][j] == "+") {
                ...
                std::uint64_t num2Digits = std::log10(arr[j+1]) + 1;
                result = result * std::pow(10, num2Digits) + arr[j+1];
            }
        }
